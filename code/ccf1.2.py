...
date_min = df_train['DATA_DATE'].min()
date_max = df_train['DATA_DATE'].max()
for d in dic:
...
#    ratio_d, corr_d = other_index(ts_mean,median_ser,1)
    ...
    if 'LABEL' in arr.columns:
        ...
        if label not in [0, 1]:
            print 'fuck'
        data_feature = np.concatenate((daily_feature, month_arr, month_mean, daily_feature_d,
                                       month_arr_d, month_mean_d, np.array([PF1]), np.array([na_days_num]), np.array([label])), axis=1)
    else:
        data_feature = np.concatenate((daily_feature, month_arr, month_mean, daily_feature_d,
                                     month_arr_d, month_mean_d, np.array([PF1]), np.array([na_days_num])), axis=1)
...
data_train.to_csv('./data/trainfeature1108.csv', header=False)
...
data_test.to_csv('./data/testfeature1108.csv', header=False)
