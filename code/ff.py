...
#参数time:输入时间序列
#参数r:前r个月与后 r个月
def variability_index(time1,r,type_n):

    if sum(time1.notnull())==0:
        time = time1.fillna(0.1)
    else:
        time = time1.dropna()
    qian_r = time[:r]
    hou_r = time[-r:]
...
#    ratio,corr=other_index(time,r)
    if type_n==0:
        return mean1,sum1,var1,jizhi,tra,trb,davg_r,dfou_r,sd,bsd,esd_s
    elif type_n in [1,3]:
        return pd.DataFrame([[mean1],[sum1],[var1],[jizhi],[tra],[trb],[davg_r],[dfou_r],[coef],[sd],[bsd],[esd_s]])
    else:
        return mean1,sum1,var1,jizhi,tra,trb,davg_r,dfou_r,coef,sd,bsd,esd_s
...
def mean_f(arr):
  ...
  #  df_daily = ts.resample('D',fill_method='ffill')
    mean1,sum1,var1,jizhi1,tra1,trb1,davg_r1,dfou_r1,sd1,bsd1,esd_s1 = feature_fun(ts,100,0)
...
#    mean4,sum4,var4,std4,median4,max4,min4,jizhi4,tra4,trb4,davg_r4,dfou_r4,coef4,sd4,bsd4,esd_s4 = feature_fun(ts_week,3,5)
    ...
    daily_feature = np.array([mean1,sum1,var1,jizhi1])#每天
  #  print daily_feature.shape
   ...
   # print month_arr.shape
    ...
    month_mean = np.array([ratio,corr,mean2,sum2,var2,jizhi2,tra2,trb2,davg_r2,dfou_r2,coef2,sd2,bsd2,esd_s2])  #月平均
...
#    week_mean = np.array([mean4,sum4,var4,std4,median4,max4,min4,jizhi4,tra4,trb4,davg_r4,dfou_r4,coef4,sd4,bsd4,esd_s4])   #季度平均
    ...
    date_feature = np.concatenate((daily_feature,month_arr,month_mean))
...
def mean_all(arr):
    ...
    group_time = group_time.fillna(group_time.mean())
   # df_month_mean = group_time.mean()
    return group_time
...
if __name__ == '__main__':
    ...
    trainout = './data/trainfeature_shao.csv'
    tstdata = './data/df_test.csv'
    tstout = './data/testfeature_shao.csv'
